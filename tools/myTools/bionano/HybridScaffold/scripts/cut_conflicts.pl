# $Id: cut_conflicts.pl 6869 2017-08-15 22:52:30Z apang $
...
my ($align1XmapFile, $align1GMFile, $align1SeqFile) = ("", "", "");
my ($maxOverhang) = 5;	
my ($breakPointFileShiftAmount) = 30;	# should be the same as the value used in AssignAlignType
...
GetOptions	(
	"align1XmapFile=s"		=>	\$align1XmapFile,
	"align1GMFile=s"		=>	\$align1GMFile,
	"align1SeqFile=s"		=>	\$align1SeqFile,
	"maxOverhang=i"			=>	\$maxOverhang,
	"breakPointFileShiftAmount=i"	=>	\$breakPointFileShiftAmount,
...
if ($modBkptStatusFile eq "")	{
	...
	if ($noQScoreFlag == 0)	{
		...
		my $align1QueryCmapRef = readAlign1Cmap2Paint($align1GMFile);
		my $align1ReferenceCmapRef = readAlign1Cmap2Paint($align1SeqFile);
		my ($align1Xmap4Ref2PaintRef, $align1Xmap4Qry2PaintRef) = readAlign1Xmap4Paint($align1XmapFile);
		...
		$conflictsAllRef = checkSupportingPaint($conflictsAllRef, $align1QueryCmapRef, $align1ReferenceCmapRef, $maxOverhang, $breakPointFileShiftAmount);
	} else	{
	...
	printUpdatedBreakPointFile($outBkptFile, $headerLinesRef, $conflictsAllRef, "auto");
} else	{
	...
	# print an updated breakpoint file
	printUpdatedBreakPointFile($outBkptFile, $headerLinesRef, $conflictsAllRef, "manual");
...
sub checkSupportingPaint	{
	my ($conflictsAllRef, $paintedQryCmapRef, $paintedRefCmapRef, $maxOverhang, $breakPointFileShiftAmount) = @_;
	...
	foreach my $theKey (keys %$conflictsAllRef)	{
		my ($xId, $refId, $qryId) = split(/\t/, $theKey);
		for (my $i = 0; $i < scalar(@{$conflictsAllRef->{$theKey}}); $i += 1)	{
			dieLog "ERROR: cut_conflicts: checkSupportingPaint: cannot find paint information for qry id = $qryId\n" if (! exists $paintedQryCmapRef->{$qryId});
			dieLog "ERROR: cut_conflicts: checkSupportingPaint: cannot find paint information for ref id = $refId\n" if (! exists $paintedRefCmapRef->{$refId});
			my $cRef = $conflictsAllRef->{$theKey}[$i];
			...
			if (! ($cRef->{refLeftBkptStatus} eq "okay" && $cRef->{qryLeftBkptStatus} eq "okay"))	{
				...
				($cRef->{refLeftBkptStatus}, $cRef->{qryLeftBkptStatus}) = conflictResolveWithAltSupport($refId, $qryId, $refLeftBkpt + $breakPointFileShiftAmount, $qryLeftBkpt + $breakPointFileShiftAmount, $cRef->{refLeftBkptStatus}, $cRef->{qryLeftBkptStatus}, $rRef, $qRef, $maxOverhang);
			...
			if (! ($cRef->{refRightBkptStatus} eq "okay" && $cRef->{qryRightBkptStatus} eq "okay"))	{
				...
				($cRef->{refRightBkptStatus}, $cRef->{qryRightBkptStatus}) = conflictResolveWithAltSupport($refId, $qryId, $refRightBkpt - $breakPointFileShiftAmount, $qryRightBkpt - $breakPointFileShiftAmount, $cRef->{refRightBkptStatus}, $cRef->{qryRightBkptStatus}, $rRef, $qRef, $maxOverhang);
			} # if we need to deal with the right bkpt
		} # for i
	} # foreach theKey
	return $conflictsAllRef;
} # checkSupportingPaint

sub conflictResolveWithAltSupport{
	my ($refId, $qryId, $refBkpt, $qryBkpt, $refBkptStatus, $qryBkptStatus, $paintedRefCmapIdRef, $paintedQryCmapIdRef, $maxOverhang) = @_;
	my $refOIndeciesRef = searchForOverlap($refBkpt, $refBkpt, $paintedRefCmapIdRef);	# should have only 1 matching that co-ordinate

	dieLog "ERROR: cut_conflicts conflictResolveWithAltSupport: cannot find the bkpt co-ordinate=$refBkpt for id=$refId in the paintedRefIdCmap array\n" if (scalar(@$refOIndeciesRef) == 0);
	my ($indexStart, $indexEnd) = (($refOIndeciesRef->[0] - $maxOverhang < 0) ? (0) : ($refOIndeciesRef->[0] - $maxOverhang), 
					($refOIndeciesRef->[0] + $maxOverhang > $#$paintedRefCmapIdRef) ? ($#$paintedRefCmapIdRef) : ($refOIndeciesRef->[0] + $maxOverhang));
	...
	dieLog "ERROR: cut_conflicts conflictResolveWithAltSupport: cannot find the bkpt co-ordinate=$qryBkpt for id=$qryId in the paintedQryIdCmap array\n" if (scalar(@$qryOIndeciesRef) == 0);
	($indexStart, $indexEnd) = (($qryOIndeciesRef->[0] - $maxOverhang < 0) ? (0) : ($qryOIndeciesRef->[0] - $maxOverhang),
					($qryOIndeciesRef->[0] + $maxOverhang > $#$paintedQryCmapIdRef) ? ($#$paintedQryCmapIdRef) : ($qryOIndeciesRef->[0] + $maxOverhang));
...
sub flagCut	{
	...
	foreach my $id (sort numeric keys %$conflictsRef)	{
...
sub paintAlign1Cmap	{
	...
	foreach my $id (keys %$xmapRef)	{
		dieLog "ERROR: cut_conflicts, paintAlign1Cmap: cannot find any cmap information, yet alignment information is available for contig id = $id\n" if (! exists $cmapRef->{$id});
...
sub readAlign1Xmap4Paint	{
	...
	open(IN, $file) or dieLog "ERROR: cut_conflicts, readAlign1Xmap4Paint: cannot open file $file: $!\n";
	while (my $line = <IN>)	{
		chomp $line;	$line =~ s/\r//g;
		...
		if ($refStartAlignLabel == -1 || $qryStartAlignLabel == -2 || $refEndAlignLabel == -1 || $qryEndAlignLabel == -1)	{
			dieLog "ERROR: cut_conflicts, readAlign1Xmap4Paint: cannot parse alignment string: $alignmentString\n\tin line $line\n";
		} # if refStartAlignLabel
...
} # readAlign1Xmap4Paint

sub readAlign1Cmap2Paint	{
	my ($file) = @_;
	# assuming that cmap entries are sorted according to their id, and then their position
	my %cmap2Paint = ();
	open(IN, $file) or dieLog "ERROR: cut_conflicts, readAlign1Cmap4Paint: cannot open file $file: $!\n";
	while (my $line = <IN>)	{
		chomp $line;	$line =~ s/\r//g;
		next if ($line =~ /^#/);
		my @content = split(/\t/, $line);
		my ($id, $siteId, $labelChannel, $position) = ($content[0], $content[3], $content[4], int($content[5]));	# remember to change the position to integer
		# cmap length line
