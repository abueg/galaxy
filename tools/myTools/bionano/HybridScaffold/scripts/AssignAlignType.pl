# $Id: AssignAlignType.pl 6871 2017-08-15 23:49:58Z apang $
...
my $breakPointFileShiftAmount = $ARGV[11];	# the amount to shift at the conflict breakpoints so that the last label of the alignment is preserved
...
printBkptFile($break_point_file, \@breakPointInfo, $breakPointFileShiftAmount);
print "Info: $0 finished successfully.\n\n";
exit 0;

sub printBkptFile	{
	my ($file, $breakPointInfoRef, $breakPointFileShiftAmount) = @_;
	my $shiftAmount = $breakPointFileShiftAmount;
	open(OUT, ">$file") or dieLog "ERROR: AssignAlignType printBkptFile: cannot write to $file: $!\n";
	...
	for (my $i = 0; $i < scalar(@{$breakPointInfoRef}); $i += 1)	{
		...
		my $toPrintLeftQryBkpt = ($bRef->{leftQryBkpt}==-1) ? ($bRef->{leftQryBkpt}) : (($bRef->{alignOrientation} == "+") ? ($bRef->{leftQryBkpt} - $shiftAmount) : ($bRef->{leftQryBkpt} + $shiftAmount));
		my $toPrintRightQryBkpt = ($bRef->{rightQryBkpt}==-1) ? ($bRef->{rightQryBkpt}) : (($bRef->{alignOrientation} == "+") ? ($bRef->{rightQryBkpt} + $shiftAmount) : ($bRef->{rightQryBkpt} - $shiftAmount));
