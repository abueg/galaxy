# $Id: align_molecules.pl 10293 2019-11-18 19:21:05Z Elam $
#!/usr/bin/perl -w
...
@$cmdRef = ("python2.7", $autoNoisePythonScript, "-x", "-y", "-T", $numThreads, "-j", $globalMaxThreads, "-l", $autoNoiseOutDir, "-t", $refAlignerDir, "-b", "$moleculesDir/$moleculesFile", "-r", "$bioNanoMapDir/$bioNanoMapFile", "-a", $deNovoOpArgsFile);
...
#################### align molecules to Bionano genome maps ####################
...
@$cmdRef = ("python2.7", $alignMolPythonScript, "-T", $numThreads, "-j", $globalMaxThreads, "-o", $alignMolBioNanoOutDir, "-t", $refAlignerDir, "-b", "$moleculesDir/$moleculesFile", "-q", "$bioNanoMapDir/$bioNanoMapFile", "-a", $deNovoOpArgsFile, "-E", "$autoNoiseOutDir/contigs/auto_noise/autoNoise1.errbin", "-p", $deNovoPipelineDir, "--split_xmap");
print "Running command: ".(join(" ", @$cmdRef))."\n";
($outResults, $errResults) = runCommand($cmdRef);
if ($jobStatus != 0)	{
	# print out error
...
# check the stdout of the alignment for "END of output" keyword
errCheckRefAligner("$alignMolBioNanoOutDir/merge/exp_refineFinal1.stdout", "END of output", "Alignment of molecules to Bionano genome map completed.", "ERROR: Alignment of molecules to Bionano genome map cannot be completed.");

#################### align molecules to hybrid scaffolds ####################
print "\nBeginning aligning molecules to hybrid scaffolds\n";
@$cmdRef = ("python2.7", $alignMolPythonScript, "-T", $numThreads, "-j", $globalMaxThreads, "-o", $alignMolHybridOutDir, "-t", $refAlignerDir, "-b", "$moleculesDir/$moleculesFile", "-q", "$hybridMapDir/$hybridMapFile", "-a", $deNovoOpArgsFile, "-E", "$autoNoiseOutDir/contigs/auto_noise/autoNoise1.errbin", "-p", $deNovoPipelineDir, "--split_xmap");
print "Running command: ".(join(" ", @$cmdRef))."\n";
($outResults, $errResults) = runCommand($cmdRef);
if ($jobStatus != 0)	{
	# print out error
...
errCheckRefAligner("$alignMolHybridOutDir/merge/exp_refineFinal1.stdout", "END of output", "Alignment of molecules to Bionano genome map completed.", "ERROR: Alignment of molecules to Bionano genome map cannot be completed.");
