# $Id: ExportAGP.pl 11203 2020-06-18 21:28:10Z jwang $
#!/usr/bin/perl -w
...
sub Usage{
	my $usage="Usage:                                                       
     ExportAGP.pl <-h> <-i xmap_file> <-o output_file>                   
	   ...
	   -t    : Trim the overlapping contigs that are next to each other on the same hybrid scaffold 
...
sub initCli(){
	...
	$INPUTS{NCBI_fasta_out_file} = $INPUTS{out_dir}.$path_sep.(stripExtension(getFileName($INPUTS{xmap_file})))."_NCBI.fasta"; #for printing fasta that is compatible with NCBI
	...
	if(defined $options{"s"} || defined $options{"c"} || defined $options{"e"}){
		if(defined $options{"s"} && defined $options{"c"} && defined $options{"e"}){		
			...
			if($enzyme{$all_cap_name}){
				...
				$INPUTS{cut_site} =$all_cap_name
			}
			$INPUTS{NCBI_cut_site} = "N"; #for printing only N-base gaps to be compatible with NCBI
		}else{
...
sub updateNGSFiles{
	...
	my $cutted_fasta_file = "$out_dir/".getFileBasename($fasta_file).".cut.fasta";
	my $cutted_ngsNameMap_file = "$out_dir/".getFileBasename($ngs_namemap_file).".cut.txt";
	...
	foreach my $key(@sort_ids){
		...
		#apparently there is off 1 difference btw alignment and seq in cut-coordinate file
		#when the cutted ngs is at the beginning or end of orig contig there is off-one error from each side
		#therefore subseq that is differs by 2bp from the orig contig is not really a subseq, so not cutting them
...
sub adjustRefStart{
	...
	my $beginGap = beginGap($qryStart, $qryEnd, $qryLen, $direction);
	#print "$qryStart\t$qryEnd\t$refStart\t$qryLen\t$direction\t$beginGap\n";
...
sub adjustRefEnd{
	...
	my $endGap = EndGap($qryStart, $qryEnd, $qryLen, $direction);
	#print "$qryStart\t$qryEnd\t$refEnd\t$qryLen\t$direction\t$endGap\n";
...
1; #only for require statement for testing