# $Id: hybridScaffold.pl 11186 2020-06-16 19:11:52Z jwang $
#!/usr/bin/perl -w
#################################################################################################################################################################################
...
#     -g    : Trim overlapping NGS sequnce during agp and fasta export
#
#################################################################################################################################################################################
...
if (defined($opts{M}))	{
	...
	if(!$opts{g}){
		...
		@$cmdRef = ($^X, "scripts/ExportAGP.pl", "-i", "$final_outputDir/$prefix.xmap", "-c", "$final_outputDir/$prefix"."_r.cmap", "-o", $agpFastaDir, "-m", $filename_key, "-s", $fastaName, "-e", $enzymeString, "-r", $cutNGSCoordFile, "-t");
	}
} elsif ($opts{N} == 2)	{
	...
	if(!$opts{g}){
		...
		@$cmdRef = ($^X, "scripts/ExportAGP.pl", "-i", "$final_outputDir/$prefix.xmap", "-c", "$final_outputDir/$prefix"."_r.cmap", "-o", $agpFastaDir, "-m", $filename_key, "-s", $fastaName, "-e", $enzymeString, "-t");	
	}
} # if opts
print "Running command: ".(join(" ", @$cmdRef))."\n";
($outResults, $errResults) = runCommand($cmdRef);
...
# new key file would exist if conflict resolution for sequence was indicated by opts{N} and opts{M}
if (defined($opts{M}) || $opts{N} == 2) {
        my $newAgpFastaDir = "";
        if (defined($opts{M}))  {
                $newAgpFastaDir = "agp_fasta"."_M$modifyNum";
        } else  {
                $newAgpFastaDir = "agp_fasta";
        } # if defined

        copy "$opts{o}/$newAgpFastaDir/$filenameKeyFile.cut.txt", "$hybridScaffoldFinalResultsDir";                                                      
} # if cut was done
...
if (defined $opts{x})	{
 ...
 if (!defined($opts{a})){
	...
	if (!defined($opts{z})){
		$opts{z} = "hybridScaffold_archive.zip"
	}
	
	my $tarZipFile = "$opts{o}/$opts{z}";
	my $tarName = "$opts{o}/$opts{z}";	$tarName =~ s/\.tar\.gz$//;	$tarName=~ s/\.zip$//; # remove the extension
	...
	if (defined($opts{x}))	{
		...
		my $tarZipDest = $tarZipSource;	$tarZipDest .= ".zip";	#$tarZipDest .= ".tar.gz";
		#@$cmdRef = ("tar", "-czf", $tarZipDest, $tarZipSource);
		@$cmdRef = ("zip", "-r", $tarZipDest, $tarZipSource);
		($outResults, $errResults) = runCommand($cmdRef);
		@$cmdRef = ();
		# error check
		...
		$tarZipDest = $tarZipSource;	$tarZipDest .= ".zip";	# $tarZipDest .= ".tar.gz";
		# @$cmdRef = ("tar", "-czf", $tarZipDest, $tarZipSource);
		@$cmdRef = ("zip", "-r", $tarZipDest, $tarZipSource);
		($outResults, $errResults) = runCommand($cmdRef);
		@$cmdRef = ();
	...
	my $tarNameRelativePath = $opts{z};	$tarNameRelativePath =~ s/\.tar\.gz$//;	$tarNameRelativePath =~ s/\.zip$//;
	print "tar file: $tarZipFile\n";
	#@$cmdRef = ( "tar", "-czf", $tarZipFile, $tarNameRelativePath);	# tar zip the directory tarName into tarZipFile			 
	@$cmdRef = ( "zip", "-r", $tarZipFile, $tarNameRelativePath);
	($outResults, $errResults) = runCommand($cmdRef);
	@$cmdRef = ();
	# error check
	errCheck($outResults, $errResults, "fail", "The tar zip command completed.", "ERROR: the tar zip command cannot be completed.");
...
sub Init{
	my $opt_string = 'hn:b:c:o:r:B:N:fxym:vM:p:q:e:z:Sw:tu:ag';
	...
	if (defined($opts{z}) && $opts{z} !~ /\.zip$/)	{
		print ("\nERROR: Please provide a valid name for a zip file (e.g. hybrid_results.zip), option -z\n");
		Usage();
	} # if opts{z}
} # Init
